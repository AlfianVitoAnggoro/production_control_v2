...
class M_DashboardGrid extends Model
{
  ...
  public function get_data_all_by_year()
  {
    $tahun = date('Y');
    $query = $this->db->query('SELECT YEAR(date_production) AS year,SUM(total_jks) AS total_jks, SUM(total_aktual) AS total_aktual FROM lhp_grid WHERE YEAR(date_production) = ' . $tahun . ' GROUP BY YEAR(date_production) ORDER BY YEAR(date_production)');
    return $query->getResultArray();
  }

  public function get_data_all_by_month($bulan)
  {
    $query = $this->db->query('SELECT MONTH(date_production) AS month,SUM(total_jks) AS total_jks, SUM(total_aktual) AS total_aktual
                                      FROM lhp_grid 
                                      WHERE MONTH(date_production) = ' . $bulan . '
  ...
  public function get_data_all_by_date($tanggal, $month)
  {
    ...
    $query = $this->db->query('SELECT date_production, SUM(total_jks) AS total_jks, SUM(total_aktual) AS total_aktual
                                    FROM		lhp_grid
                                    WHERE		date_production = \'' . $tanggal . '\' AND MONTH(date_production) = \'' . $month . '\'
  ...
  public function get_data_grup($month)
  {
    ...
    $query = $this->db->query('SELECT DISTINCT(lhp_grid.grup)
                                    FROM lhp_grid
                                    WHERE MONTH(lhp_grid.date_production) = ' . $bulan . '
  ...
  {
    $month = idate('m', strtotime($month . '- 1 month'));
