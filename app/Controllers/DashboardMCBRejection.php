...
use App\Models\M_DashboardMCBRejection;

class DashboardMCBRejection extends BaseController
{
    public function __construct()
    {
        $this->M_DashboardMCBRejection = new M_DashboardMCBRejection();
    ...
    public function dashboard_reject_mcb()
    {
        ...
        $data_reject_by_month = $this->M_DashboardMCBRejection->get_data_reject_internal_by_month(idate('m',strtotime($bulan)), $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardMCBRejection->get_data_rejection_by_month($i, $child_filter);
            if (!empty($data_all)) {
        ...
        $data_line = $this->M_DashboardMCBRejection->get_data_total_reject_line_by_month($bulan);
        ...
        $data_year = $this->M_DashboardMCBRejection->get_year_to_date_rejection($child_filter);
        ...
        while (strtotime($start1) <= strtotime($now)) {
            $data_average_reject_by_date_all_line = $this->M_DashboardMCBRejection->get_data_reject_all_line_by_date($start1, $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardMCBRejection->get_data_average_reject_by_month($i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_reject = $d_all['total_reject'];
                    $total_produksi = $d_all['total_produksi'];
        ...
        while (strtotime($start2) <= strtotime($now)) {
            for ($i=1; $i <= 7; $i++) { 
                $data_reject_per_line_by_date = $this->M_DashboardMCBRejection->get_data_reject_all_line($start2, $i);
        ...
        for ($h=1; $h <= 12; $h++) { 
            for ($i=1; $i <= 7; $i++) {
                $data_reject_all_line_by_month = $this->M_DashboardMCBRejection->get_data_reject_all_line_by_month($h, $i);
        ...
        return view('dashboardCutting/dashboard_lhp_cuttingMCB_rejection', $data);
    }
    ...
    public function get_detail_rejection()
    {
        ...
        $data['data_jenis_reject_internal'] = $this->M_DashboardMCBRejection->get_qty_jenis_reject_internal($date, $line);
        $data['data_jenis_reject_eksternal'] = $this->M_DashboardMCBRejection->get_qty_jenis_reject_eksternal($date, $line);
        $data['data_jenis_reject_internal_by_month'] = $this->M_DashboardMCBRejection->get_data_reject_internal_by_month(idate('m',strtotime($date)), $line);
        $data['data_jenis_reject_eksternal_by_month'] = $this->M_DashboardMCBRejection->get_data_reject_eksternal_by_month(idate('m',strtotime($date)), $line);

        echo json_encode($data);    
    }

    public function dashboard_reject_mcb_qc()
    {
        date_default_timezone_set('Asia/Jakarta');
        $start = date('Y-m-01');
        $start1 = date('Y-m-01');
        ...
        $data_reject_by_month = $this->M_DashboardMCBRejection->get_data_reject_by_month(idate('m',strtotime($bulan)), $child_filter);
