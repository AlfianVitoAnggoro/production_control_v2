...
class DashboardWetFinishing extends BaseController
{
    ...
    public function dashboard_lhp_wet_finishing()
    {
        ...
        if ($jenis_dashboard == 1 AND ($parent_filter == 'line' OR $parent_filter == null) AND ($child_filter == null OR $child_filter == 0) AND $baby_filter == 'average') {
            ...
            for ($h=1; $h <= 12; $h++) { 
                for ($i=8; $i <= 9; $i++) {
                    $data_all_line_by_month = $this->M_DashboardWetFinishing->get_data_all_line_by_month($h, $i);
            ...
            if (!empty($data_grup_line_year)) {
                for ($i=1; $i <= 12 ; $i++) { 
                    ...
                    foreach ($data_grup_line_year as $d_grup_line) {
                        ...
                        $data_all_grup_month = $this->M_DashboardWetFinishing->get_data_line_by_grup_month($i, $child_filter, $grup_month);
        ...
        for ($i=8; $i <= 9; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month($current_date, $i);
            if (!empty($data_all)) {
        ...
        for ($i=8; $i <= 9; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month($previous_date, $i);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data['data_all_month'] = [];
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardWetFinishing->get_data_all_line_by_month($i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_plan = $d_all['total_plan'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        $data_year = $this->M_DashboardWetFinishing->get_data_all_line_by_year($child_filter);
