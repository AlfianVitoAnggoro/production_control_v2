...
use App\Models\M_DashboardAssyRejectionQC;

class DashboardAssyRejectionQC extends BaseController
{
    public function __construct()
    {
        $this->M_DashboardAssyRejectionQC = new M_DashboardAssyRejectionQC();
    ...
    public function get_detail_rejection()
    {
        ...
        $data['total_aktual_by_month'] = $this->M_DashboardAssyRejectionQC->get_total_aktual_by_month(idate('m',strtotime($date)), $line);
        $data['total_aktual_by_date'] = $this->M_DashboardAssyRejectionQC->get_total_aktual_by_date($date, $line);

        $data['data_jenis_reject'] = $this->M_DashboardAssyRejectionQC->get_qty_jenis_reject($date, $line);
        $data['data_jenis_reject_by_month'] = $this->M_DashboardAssyRejectionQC->get_data_reject_by_month(idate('m',strtotime($date)), $line);

        $data['data_reject_by_jenis_reject'] = $this->M_DashboardAssyRejectionQC->get_detail_rejection_by_jenis($jenis_reject, $date, $line);
        $data['data_reject_by_type_battery'] = $this->M_DashboardAssyRejectionQC->get_detail_rejection_by_type_battery($jenis_reject, $date, $line);
        $data['data_reject_by_grup'] = $this->M_DashboardAssyRejectionQC->get_detail_rejection_by_grup($jenis_reject, $date, $line);

        $data['data_jenis_reject_by_type_battery'] = $this->M_DashboardAssyRejectionQC->get_jenis_reject_by_type_battery($type_battery, $date, $line);
        $data['data_kategori_reject_by_type_battery'] = $this->M_DashboardAssyRejectionQC->get_kategori_reject_by_type_battery($type_battery, $date, $line);
        $data['data_grup_reject_by_type_battery'] = $this->M_DashboardAssyRejectionQC->get_grup_reject_by_type_battery($type_battery, $date, $line);

        $data['data_jenis_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionQC->get_jenis_reject_by_grup_shift($grup, $shift, $date, $line);
        $data['data_kategori_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionQC->get_kategori_reject_by_grup_shift($grup, $shift, $date, $line);
        $data['data_battery_reject_by_grup_shift'] = $this->M_DashboardAssyRejectionQC->get_battery_reject_by_grup_shift($grup, $shift, $date, $line);

        $data['data_all_detail_kategori_rejection_by_date'] = $this->M_DashboardAssyRejectionQC->get_all_detail_kategori_rejection_by_date($date, $line);
        $data['data_all_detail_battery_rejection_by_date'] = $this->M_DashboardAssyRejectionQC->get_all_detail_battery_rejection_by_date($date, $line);
        $data['data_all_detail_grup_rejection_by_date'] = $this->M_DashboardAssyRejectionQC->get_all_detail_grup_rejection_by_date($date, $line);

        $data['data_reject_by_jenis_reject_by_month'] = $this->M_DashboardAssyRejectionQC->get_detail_rejection_by_jenis_by_month($jenis_reject, idate('m',strtotime($date)), $line);
        $data['data_reject_by_type_battery_by_month'] = $this->M_DashboardAssyRejectionQC->get_detail_rejection_by_type_battery_by_month($jenis_reject, idate('m',strtotime($date)), $line);

        $data['data_all_detail_kategori_rejection_by_month'] = $this->M_DashboardAssyRejectionQC->get_all_detail_kategori_rejection_by_month(idate('m',strtotime($date)), $line);
        $data['data_all_detail_battery_rejection_by_month'] = $this->M_DashboardAssyRejectionQC->get_all_detail_battery_rejection_by_month(idate('m',strtotime($date)), $line);
        $data['data_all_detail_grup_rejection_by_month'] = $this->M_DashboardAssyRejectionQC->get_all_detail_grup_rejection_by_month(idate('m',strtotime($date)), $line);

        $data['detail_summary_rejection'] = $this->M_DashboardAssyRejectionQC->get_detail_summary_rejection($jenis_reject, $date, $line);
    ...
    public function dashboard_reject_assy_qc()
    {
        ...
        // $total_data_reject_by_month = $this->M_DashboardAssyRejectionQC->get_total_data_reject_by_month(idate('m',strtotime($bulan)));
        // $total_aktual_by_month = $this->M_DashboardAssyRejectionQC->get_total_aktual_by_month(idate('m',strtotime($bulan)));
        $data_reject_by_month = $this->M_DashboardAssyRejectionQC->get_data_reject_by_month(idate('m',strtotime($bulan)), $child_filter);
        ...
        $data_jenis_reject = $this->M_DashboardAssyRejectionQC->get_jenis_reject_by_month($start, $child_filter);

        if (!empty($data_jenis_reject)) {
            while (strtotime($start) <= strtotime($now)) {
                foreach ($data_jenis_reject as $d_jenis_reject) {
                    $data_jenis_reject_by_date = $this->M_DashboardAssyRejectionQC->get_data_reject_by_date($start, $d_jenis_reject['jenis_reject'], $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardAssyRejectionQC->get_data_rejection_by_month($i, $child_filter);
            if (!empty($data_all)) {
        ...
        $data_line_by_date = $this->M_DashboardAssyRejectionQC->get_data_total_reject_line_by_date($bulan);
        ...
        $data_line = $this->M_DashboardAssyRejectionQC->get_data_total_reject_line_by_month($bulan);
        ...
        $data_year = $this->M_DashboardAssyRejectionQC->get_year_to_date_rejection($child_filter);
        ...
        while (strtotime($start1) <= strtotime($now)) {
            $data_average_reject_by_date_all_line = $this->M_DashboardAssyRejectionQC->get_data_reject_all_line_by_date($start1, $child_filter);
        ...
        for ($i=1; $i <= 12; $i++) { 
            $data_all = $this->M_DashboardAssyRejectionQC->get_data_average_reject_by_month($i, $child_filter);
            if (!empty($data_all)) {
                foreach($data_all as $d_all) {
                    $total_reject = $d_all['total_reject'];
                    $total_aktual = $d_all['total_aktual'];
        ...
        while (strtotime($start2) <= strtotime($now)) {
            for ($i=1; $i <= 7; $i++) { 
                $data_reject_per_line_by_date = $this->M_DashboardAssyRejectionQC->get_data_reject_all_line($start2, $i);
        ...
        for ($h=1; $h <= 12; $h++) { 
            for ($i=1; $i <= 7; $i++) {
                $data_reject_all_line_by_month = $this->M_DashboardAssyRejectionQC->get_data_reject_all_line_by_month($h, $i);
        ...
        $data_jenis_reject_persentase = $this->M_DashboardAssyRejectionQC->get_jenis_reject_by_month($start3, $child_filter);
        ...
        if (!empty($data_jenis_reject_persentase)) {
            while (strtotime($start3) <= strtotime($now)) {
                foreach ($data_jenis_reject_persentase as $d_jenis_reject) {
                    $data_jenis_reject_by_date_persentase = $this->M_DashboardAssyRejectionQC->get_data_reject_by_date($start3, $d_jenis_reject['jenis_reject'], $child_filter);
                    $data_reject_per_line_by_date = $this->M_DashboardAssyRejectionQC->get_data_reject_all_line($start3, $child_filter);
