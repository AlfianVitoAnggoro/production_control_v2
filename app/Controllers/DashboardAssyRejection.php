...
class DashboardAssyRejection extends BaseController
{
    ...
    public function dashboard_reject_assy()
    {
        ...
        $data_reject_by_month = $this->M_DashboardAssyRejection->get_data_reject_by_month(idate('m',strtotime($bulan)), $child_filter);
        ...
        for ($h=1; $h <= 12; $h++) { 
            for ($i=1; $i <= 7; $i++) {
                $data_reject_all_line_by_month = $this->M_DashboardAssyRejection->get_data_reject_all_line_by_month($h, $i);
    ...
    public function get_detail_rejection()
    {
        ...
        $data['data_jenis_reject_by_month'] = $this->M_DashboardAssyRejection->get_data_reject_by_month(idate('m',strtotime($date)), $line);
        ...
        $data['data_reject_by_jenis_reject_by_month'] = $this->M_DashboardAssyRejection->get_detail_rejection_by_jenis_by_month($jenis_reject, idate('m',strtotime($date)), $line);
        $data['data_reject_by_type_battery_by_month'] = $this->M_DashboardAssyRejection->get_detail_rejection_by_type_battery_by_month($jenis_reject, idate('m',strtotime($date)), $line);

        $data['data_all_detail_kategori_rejection_by_month'] = $this->M_DashboardAssyRejection->get_all_detail_kategori_rejection_by_month(idate('m',strtotime($date)), $line);
