...
use App\Models\M_DashboardPasting;

class DashboardPasting extends BaseController
{
  public function __construct()
  {
    $this->M_DashboardPasting = new M_DashboardPasting();
    $this->session = \Config\Services::session();
  }

  // public function index()
  // {
  //   return view('dashboardPasting/home');
  // }

  public function dashboard_lhp_pasting()
  {
    ...
    $data['bulan'] = $bulan;
    $data_year = $this->M_DashboardPasting->get_data_all_by_year($bulan);
    ...
    $data['data_all_month'] = [];
    for ($i = 1; $i <= 12; $i++) {
      $data_all_month = $this->M_DashboardPasting->get_data_all_by_month($i, $bulan);
    ...
    while (strtotime($start) <= strtotime($now)) {
      $data_all = $this->M_DashboardPasting->get_data_all_by_date($start, $bulan);
      if (!empty($data_all)) {
    ...
    // $data_all_mp = $this->M_DashboardPasting->get_data_all_mp_by_current_month($bulan);
    ...
    // $data_all_mp = $this->M_DashboardPasting->get_data_all_mp_by_previous_month($bulan);
    // if (!empty($data_all_mp)) {
    //   foreach ($data_all_mp as $damp) {
    //     if ($damp['operator_name'] !== NULL && $damp['operator_name'] !== 0) {
    ...
    $data_by_grup = $this->M_DashboardPasting->get_data_grup($start);
    $data['data_all_grup'] = $data_by_grup;

    if (!empty($data_by_grup)) {
      while (strtotime($start) <= strtotime($now)) {
        foreach ($data_by_grup as $d_grup_grup) {
          $grup = $d_grup_grup['grup'];
          $data_all_grup = $this->M_DashboardPasting->get_data_by_grup($start, $grup);
    ...
    if (count($data_by_grup) > 0) {
      foreach ($data_by_grup as $d_by_grup) {
        $data_all_grup_current_month = $this->M_DashboardPasting->get_data_all_grup_by_current_month($bulan, $d_by_grup['grup']);
    ...
    $data_by_grup_previous_month = $this->M_DashboardPasting->get_data_grup(date('d-m-Y', strtotime($start . '- 1 month')));
    ...
    if (count($data_by_grup) > 0) {
      foreach ($data_by_grup as $d_by_grup) {
        $data_all_grup_previous_month = $this->M_DashboardPasting->get_data_all_grup_by_previous_month($bulan, $d_by_grup['grup']);
    ...
    return view('dashboardPasting/dashboard_lhp_pasting', $data);
  }
  ...
  public function get_data_line_stop()
  {
    ...
    $data = $this->M_DashboardPasting->get_data_line_stop($tanggal);
