  ...
  <section class="content p-0">
    ...
    <div class="row m-0" style="height: 90vh">
      <div class="col-2 px-1">
        ...
        <div class="row m-0" style="height: calc(100% - 53vh - 7px)">
        ...
        <div class="row m-0" style="height: calc(8vh - 4px);">
          <div class="col px-1 bg-white" style="height: 100%;">
            <div class="d-flex justify-content-center align-items-center flex-column" style="height: 100%;">
              <h1 class="text-center m-0" style="font-size: 20px">Skill Map</h1>
          ...
          <div class="row m-0" style="height: 45vh;">
            <div class="col p-0">
              <div class="d-flex" style="width: 100%">
                <div class="d-flex justify-content-center align-items-center flex-column mb-2 mx-1" style="border-radius: 5px; background-color: #22A699; color: white; width: 150px">
        ...
        <div class="row m-0" style="height: 22.5vh;">
          ...
          <div class="col"> -->
            <div class="d-flex" style="width: 100%; height: calc(100% - 8px)">
              ...
              <div class="row row-cols-xl-4 m-0" style="width: calc(100% - 157px); background-color: #F86F03; border-radius: 5px;">
                <!-- <div class="col" style="width: 150px">
          ...
          <?php for ($i = 0; $i < count($line); $i++) { ?>
            <div class="col px-1 mb-2">
              <table class="table bg-white" style="height: 100%; border-radius: 5px">
                <thead>
                  <tr>
                    <th style="font-size: 20px" class="py-1 px-0 text-center"><?= $lineAct[$line[$i]] ?></th>
                  ...
                  <?php if (array_key_exists($line[$i], $data_mp_tidak_hadir) ? count($data_mp_tidak_hadir[$line[$i]]) : 0) {
                    ...
                    foreach ($data_mp_tidak_hadir[$line[$i]] as $dmth) { ?>
                      <tr id="add_mp_tidak_masuk_<?= $line[$i] ?>_<?= $temp_index_add ?>" style="border-bottom: <?= $temp_index_add === 2 ? 'transparent' : '' ?>;" data-bs-toggle="modal" data-bs-target=".modal_add_mp_tidak_hadir" onclick="btn_add_mp_tidak_hadir('<?= $line[$i] ?>', <?= $temp_index_add ?>)">
                        <td class="p-0 mp_cuti_<?= $line[$i] ?>" colspan="2" style="font-size: 20px; white-space: nowrap; max-width: 140px; overflow: hidden; text-overflow: ellipsis;">
                          <?= $dmth['nama'] ?>
                ...
                <?php if (count($data_mp_tidak_hadir_indirect) > 0) {
                  ...
                  foreach ($data_mp_tidak_hadir_indirect as $dmth) { ?>
                    <tr id="add_mp_tidak_masuk_indirect_<?= $temp_index_add ?>" style="border-bottom: <?= $temp_index_add === 2 ? 'transparent' : '' ?>;" data-bs-toggle="modal" data-bs-target=".modal_add_mp_tidak_hadir" onclick="btn_add_mp_tidak_hadir('indirect', <?= $temp_index_add ?>)">
                      <td class="p-0 mp_cuti_indirect" colspan="2" style="font-size: 20px; white-space: nowrap; max-width: 140px; overflow: hidden; text-overflow: ellipsis;">
                        <?= $dmth['nama'] ?>
              ...
              if (cutiElement == null) {
                if (count < data.length) {
                  ...
                  document.querySelector(`#add_mp_tidak_masuk_${line}_${index_add}`).innerHTML = `
                    <td class="p-0 mp_cuti_${line}" colspan="2" style="font-size: 20px; white-space: nowrap; max-width: 140px; overflow: hidden; text-overflow: ellipsis;">${data[count].nama}</td>
            ...
            for (let index_add = 0; index_add < data.length; index_add++) {
              ...
              if (index_add < 3) {
                if (cutiElement == null) {
                  console.log(document.querySelector(`#add_mp_tidak_masuk_${line}_${index_add}`));
                  document.querySelector(`#add_mp_tidak_masuk_${line}_${index_add}`).innerHTML = `
                    <td class="p-0 mp_cuti_${line}" colspan="2" style="font-size: 20px; white-space: nowrap; max-width: 140px; overflow: hidden; text-overflow: ellipsis;">${data[index_add].nama}</td>
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          add_mp_tidak_masukElement.innerHTML = `
            <td class="p-0 mp_cuti_${line}" colspan="2" style="font-size: 20px; white-space: nowrap; max-width: 140px; overflow: hidden; text-overflow: ellipsis;">
              ${data?.[0]?.nama}
