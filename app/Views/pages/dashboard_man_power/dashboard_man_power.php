  ...
  <section class="content p-0">
              ...
              <?php if (strpos($di['requirement'], 'Tidak Baca') === 0) { ?>
                <div>
                  <div class="d-flex justify-content-center align-items-top">
                    ...
                    <select name="group_indirect[]" id="group_indirect" class="form-select p-0 ps-5 ms-1" style="height: 20px; font-size: 12px; width: 50px" onchange="changeGroup('indirect', '<?= $sub_bagian ?>')">
    ...
    <!-- <div class="col">
        <div class="box mb-2" style="box-shadow: 0px 0px 10px rgba(0,0,0,0.2); border-radius: 5px">
          <div class="fx-card-item">
            <div class="fx-card-content">
              <div style="background-color: <?= "" //$color[$d_gmp['skill']] 
                                            ?>; border-radius: 5px 5px 0px 0px">
                <h4 class="box-title mb-0 text-center p-1 fw-bold"><?= "" //$d_gmp['mesin'] 
                                                                    ?></h4>
                <div class="bg-danger d-flex align-items-center justify-content-center p-0" style="border-radius: 50%; height: 30px; width: 30px">25%</div>
              </div>
              <h5 class="box-title mb-0 text-center"><?= "" //$d_gmp['nama'] 
                                                      ?></h5>
            </div>
            <div class="fx-card-avatar mt-2" style="width: 155px; height: 200px">
              <img src="<?= "" //base_url() 
                        ?>uploads/<?= "" //$d_gmp['foto'] 
                                  ?>" alt="" style="width: 100%; border-radius: 0px 0px 5px 5px">
            </div>
            <div class="fx-card-footer px-3 py-2 d-flex justify-content-between align-items-center">
              <h5 class="m-0"><?= "" //sprintf('%04d', $d_gmp['npk']) 
                              ?></h5>
              <div style="background-color: <?= "" //$color[$d_gmp['skill']] 
                                            ?>; border-radius: 50%; width: 24px; height: 24px"></div>
            </div>
...
<script>
        ...
        if (data.data_mesin) {
          ...
          data.data_indirect.forEach(di => {
            if (di.requirement === 'Tidak Baca') {
              // document.querySelector(`#mesin_${line}_${index_mesin}`).innerHTML = msn.mesin;
              document.querySelector(`#nama_mp_${line}_${index_mesin}`).value = data?.data_group_man_power_kasubsie?.[groupElement.value]?.[di.mesin]?.['nama'] ?? '';
              ...
              document.querySelector(`#nama_mp_${line}_${index_mesin}`).value = data?.data_group_man_power_indirect?.[groupElement.value]?.[di.mesin]?.['nama'] ?? '';
  ...
  function edit_group_man_power(line, index) {
    ...
    let mesin = document.querySelector('#mesin_form');
